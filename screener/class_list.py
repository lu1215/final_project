...
class Summary():
	def NCBI_gene_summary(gene):
		...
		row_count = cursor_summary.execute("SELECT `Symbol`,`Synonyms`,`chromosome`,`description`,`Other_designations` FROM `NCBI_gene_info_20180201` WHERE `Symbol` = '%s'"%(gene))
		summary_result = cursor_summary.fetchall()[0] if row_count > 0 else ''
